...
export function GET() {
  ...
  return json({
    ...
    admission: live ? `tokens launched in the last ${config.backfillDays} days plus every new launch; a token enters the study once its ${SITE.labelHours}h peak market cap is known` : 'all simulated launches',
    peak_market_cap: live
      ? `highest USD price in the first ${SITE.labelHours}h × total supply (fully diluted): Pons tokens from every curve trade (quote paid ÷ tokens, in USD at the trade hour); other venues from hourly candle highs`
      : 'simulated',
    ...
    holders_source: live ? `replayed from ERC-20 Transfer events up to ${SITE.holderSampleHours}h after launch, long before the ${SITE.labelHours}h outcome is known` : 'simulated',
