 ...
 */
export async function holdersAt(rpc: Pick<RpcClient, 'getLogs'>, token: string, fromBlock: number, toBlock: number): Promise<number> {
  ...
  let holders = 0;
  for (const balance of balances.values()) if (balance > 0n) holders++;
  return holders;
}
